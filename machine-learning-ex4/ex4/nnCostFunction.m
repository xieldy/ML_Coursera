...
h = [ones(size(h,1),1) h];
h = transpose(sigmoid(h * transpose(Theta2)));
ytmp = y;
y = zeros(size(h));
for i=[1:size(ytmp,1)]
	y(ytmp(i),i) = 1;
...
d3 = a3 - y;
d2 = transpose(Theta2)*d3.*transpose(sigmoidGradient(z2));
d2 = d2(2:end,:);
...
Theta1_grad = (1.0/m)*(d2*a1)+reg1_grad;
Theta2_grad = (1.0/m)*(d3*a2)+reg2_grad;
